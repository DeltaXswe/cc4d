...
/**
 * Test d'integrazione per le operazioni relative alle caratteristiche non archiviate di una
 * macchine
 */
...
public class CharacteristicsTests {
  ...
  private static void prepareContext(
    ...
    deviceId = deviceRepository.save(new DeviceEntity("macchina", false, false, "a")).getId();
    ...
    characteristic2Id =
        characteristicRepository
            .save(new CharacteristicEntity(deviceId, "pressione", 100d, 10d, true, 0, false))
            .getId();

    archivedCharacteristicId =
        characteristicRepository
            .save(new CharacteristicEntity(deviceId, "frequenza", 100d, 10d, true, 0, true))
            .getId();

    archivedDeviceId =
        deviceRepository.save(new DeviceEntity("macchina", true, false, "a")).getId();

    archivedDeviceCharacteristicId =
        characteristicRepository
            .save(
                new CharacteristicEntity(
                    archivedDeviceId, "temperatura", 98d, -13d, true, 0, false))
            .getId();
  }
   ...
   *
   * @throws Exception se la macchina non esiste, è archiviata o non vengono restituite le
   *     caratteristiche attese
   */
  @Test
  void testGetUnarchivedCharacteristics() throws Exception {
    var characteristic1 = new JSONObject().put("id", characteristic1Id).put("name", "temperatura");

    var characteristic2 = new JSONObject().put("id", characteristic2Id).put("name", "pressione");

    var response = new JSONArray().put(characteristic1).put(characteristic2);

    this.mockMvc
        .perform(get("/devices/" + deviceId + "/characteristics"))
        .andDo(print())
  ...
  /**
   * Testa l'ottenimento delle caratteristiche non archiviate di una macchina che possiede solo
   * caratteristiche archiviate
   *
   * @throws Exception se viene ottenuta qualche caratteristica, la macchina non esiste o è
   *     archiviata
   */
  @Test
  ...
  void testGetFromArchivedDeviceError() throws Exception {
    JSONObject response = new JSONObject().put("errorCode", "deviceNotFound");

    this.mockMvc
        .perform(get("/devices/" + archivedDeviceId + "/characteristics"))
        .andDo(print())
  ...
  void testDeviceNotFoundError() throws Exception {
    ...
    this.mockMvc
        .perform(get("/devices/1/characteristics"))
        .andDo(print())
        .andExpect(status().isNotFound())
        .andExpect(content().json(response.toString()));
   ...
   *
   * @throws Exception se la caratteristica è inesistente, è archiviata o non vengono restituiti i
   *     limiti attesi
   */
  @Test
  void testGetCharacteristicLimits() throws Exception {
    JSONObject response =
        new JSONObject().put("lowerLimit", -13d).put("upperLimit", 98d).put("mean", 42.5);

    this.mockMvc
        .perform(get("/devices/" + deviceId + "/characteristics/" + characteristic1Id + "/limits"))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().json(response.toString()));
  }
  ...
  void testCharacteristicNotFoundError() throws Exception {
    ...
    this.mockMvc
        .perform(get("/devices/" + deviceId + "/characteristics/1/limits"))
        .andDo(print())
        .andExpect(status().isNotFound())
        .andExpect(content().json(response.toString()));
   ...
   *
   * @throws Exception se la caratteristica non esiste, non è archiviata o non viene rilevato
   *     l'errore
   */
  @Test
  ...
  /**
   * Testa l'ottenimento dei limiti tecnici di una caratteristica non archiviata di una macchina
   * archiviata
   *
   * @throws Exception se non viene rilevato l'errore
   */
