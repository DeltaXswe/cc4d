...
class DevicesTests {
  ...
  @Autowired private DevicesController devicesController;
   ...
   */
  @BeforeEach
  private void prepareContext(@Autowired DeviceRepository deviceRepository) {
   ...
   */
  @AfterEach
  private void deleteAll(@Autowired DeviceRepository deviceRepository) {
  ...
  public void testGetAllUnarchivedDevices() throws Exception {
    JSONObject d1 = new JSONObject().put("id", deviceId1).put("name", "macchina1");
    JSONObject d2 = new JSONObject().put("id", deviceId2).put("name", "macchina2");
