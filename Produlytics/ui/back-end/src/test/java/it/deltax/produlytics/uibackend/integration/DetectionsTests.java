...
public class DetectionsTests {
   ...
   * Il costruttore
   *
   * @throws JSONException se viene inserita una chiave duplicata nella rappresentazione JSON delle
   *     rilevazioni
   */
  public DetectionsTests() throws JSONException {
    detection1 = new JSONObject().put("creationTime", 1).put("value", 100d).put("outlier", false);

    detection2 = new JSONObject().put("creationTime", 2).put("value", 200d).put("outlier", false);

    detection3 = new JSONObject().put("creationTime", 3).put("value", 300d).put("outlier", false);

    detection4 = new JSONObject().put("creationTime", 4).put("value", 400d).put("outlier", false);
  }
  ...
  private static void prepareContext(
    ...
    deviceId = deviceRepository.save(new DeviceEntity("macchina", false, false, "x")).getId();
    ...
    archivedCharacteristicId =
        characteristicRepository
            .save(new CharacteristicEntity(deviceId, "temperatura", 98d, -13d, true, 0, true))
            .getId();

    for (int i = 1; i < 5; ++i) {
      detectionRepository.save(
          new DetectionEntity(
              Instant.ofEpochMilli(i), characteristicId, deviceId, 100d * i, false));
    }
  }
   ...
   *
   * @throws Exception se la caratteristica è inesistente, archiviata o si ottiene un risultato
   *     diverso da quello atteso
   */
  @Test
  void testGetWithNoFilter() throws Exception {
    JSONArray detections =
        new JSONArray().put(detection1).put(detection2).put(detection3).put(detection4);

    JSONObject response =
        new JSONObject().put("detections", detections).put("nextOld", null).put("nextNew", 4);

    this.mockMvc
        .perform(
            get("/devices/" + deviceId + "/characteristics/" + characteristicId + "/detections"))
        .andDo(print())
  ...
  void testGetWithLimit() throws Exception {
    JSONArray detections = new JSONArray().put(detection3).put(detection4);

    JSONObject response =
        new JSONObject().put("detections", detections).put("nextOld", 3).put("nextNew", 4);
  ...
  /**
   * Testa l'ottenimento delle rilevazioni di una caratteristica meno recenti di un determinato
   * istante
   *
   * @throws Exception se la caratteristica è inesistente, archiviata o si ottiene un risultato
   *     diverso da quello atteso
  ...
  void testGetWithOlder() throws Exception {
    JSONArray detections = new JSONArray().put(detection1).put(detection2).put(detection3);

    JSONObject response =
        new JSONObject().put("detections", detections).put("nextOld", null).put("nextNew", 3);
  ...
  /**
   * Testa l'ottenimento delle rilevazioni di una caratteristica più recenti di un determinato
   * istante
   *
   * @throws Exception se la caratteristica è inesistente, archiviata o si ottiene un risultato
   *     diverso da quello atteso
  ...
  void testGetWithNewer() throws Exception {
    JSONArray detections = new JSONArray().put(detection2).put(detection3).put(detection4);

    JSONObject response =
        new JSONObject().put("detections", detections).put("nextOld", 2).put("nextNew", 4);
  ...
  /**
   * Testa l'ottenimento delle rilevazioni di una caratteristica, applicando un filtro di ricerca
   * per cui viene restituita una lista vuota
   *
   * @throws Exception se la caratteristica è inesistente, archiviata o si ottiene un risultato
   *     diverso da quello atteso
   */
  ...
  void testCharacteristicNotFoundError() throws Exception {
    ...
    this.mockMvc
        .perform(get("/devices/" + deviceId + "/characteristics/1/detections"))
        .andDo(print())
