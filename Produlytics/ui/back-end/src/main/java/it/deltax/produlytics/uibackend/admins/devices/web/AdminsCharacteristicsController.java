...
public class AdminsCharacteristicsController {
   ...
   *     caratteristica
   * @param getCharacteristicsUseCase l'interfaccia per il caso d'uso di ottenimento della lista
   *     delle caratteristiche di una macchina
   * @param updateCharacteristicArchiveStatusUseCase l'interfaccia per il caso d'uso di modifica
   *     dello stato di archiviazione della caratteristica di una macchina
   * @param updateCharacteristicUseCase l'interfaccia per il caso d'uso di modifica della
  ...
  public ResponseEntity<Map<String, Integer>> insertCharacteristic(
      @PathVariable("deviceId") int deviceId, @RequestBody NewCharacteristic characteristic)
      throws BusinessException {
    return ResponseEntity.ok(
        Map.of("id", this.insertCharacteristicUseCase.insertByDevice(deviceId, characteristic)));
  }
   ...
   */
  @GetMapping("")
  public List<Characteristic> getCharacteristics(@PathVariable("deviceId") int deviceId)
      throws BusinessException {
  ...
  public ResponseEntity<String> updateCharacteristic(
      ...
      @RequestBody JsonNode body)
      throws BusinessException {
    CharacteristicToUpdate.CharacteristicToUpdateBuilder toUpdateBuilder =
    ...
    if (body.hasNonNull("lowerLimit")) {
      toUpdateBuilder =
          toUpdateBuilder.withLowerLimit(OptionalDouble.of(body.get("lowerLimit").asDouble()));
    }

    if (body.hasNonNull("upperLimit")) {
      toUpdateBuilder =
          toUpdateBuilder.withUpperLimit(OptionalDouble.of(body.get("upperLimit").asDouble()));
    }

    if (body.hasNonNull("sampleSize")) {
      toUpdateBuilder =
          toUpdateBuilder.withSampleSize(OptionalInt.of(body.get("sampleSize").asInt()));
    }
