...
public class ControlLimitsCalculatorTest {
	...
	void testMeanStddevOnly() {
		...
		FindLimitsPort findLimitsPort = characteristicId -> {
	...
	void testTechnicalLimitsOnly() {
		...
		ControlLimitsCalculator calculator = new ControlLimitsCalculatorImpl(findLimitsPort);
		assert calculator.calculateControlLimits(findCharacteristicId).equals(new ControlLimits(10, 100));
	}
	...
	void testNone() {
		...
		ControlLimitsCalculator calculator = new ControlLimitsCalculatorImpl(findLimitsPort);
		assertThrows(RuntimeException.class, () -> calculator.calculateControlLimits(findCharacteristicId));
	}
}
