...
public class DetectionsPersistenceAdapterByCharacteristic implements ListDetectionsByCharacteristicPort {
    ...
    public List<DetectionLight> listByCharacteristic(long machineId, String characteristicCode, Optional<Long> createdAfter) {
        ...
        if (createdAfter.isPresent()) {
            detections = repo.findByMacchinaAndCaratteristicaAndCreazioneUtcGreaterThan(machineId, characteristicCode, createdAfter.get(), Sort.by("creazione_utc"));
        } else {
            detections = repo.findByMacchinaAndCaratteristica(machineId, characteristicCode, Sort.by("creazione_utc"));
        }
